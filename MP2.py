...
def mapper():
    ...
    for values in parseInput():
        ...
        try:
            destination = (float(values[12]), float(values[13]))
            destination_neighborhood = findNeighborhood(destination, index, neighborhoods)
            if pickup_neighborhood!=-1 and destination_neighborhood!=-1:
                agg[pickup_neighborhood,destination_neighborhood] = agg.get((pickup_neighborhood,destination_neighborhood), 0) + 1
        except:
            pass


    for item in agg.iteritems():
        trip='('+neighborhoods[item[0][0]][0]+','+neighborhoods[item[0][1]][0]+')'
        print '%s\t%s' % (trip, item[1])
